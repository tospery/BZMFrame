...
/// 控制图片在UIButton里的位置，默认为BZMButtonImagePositionLeft
typedef NS_ENUM(NSUInteger, BZMButtonImagePosition) {
    BZMButtonImagePositionTop,             // imageView在titleLabel上面
    BZMButtonImagePositionLeft,            // imageView在titleLabel左边
    BZMButtonImagePositionBottom,          // imageView在titleLabel下面
    BZMButtonImagePositionRight,           // imageView在titleLabel右边
};
 ...
 *  @warning BZMButton 重新定义了 UIButton.titleEdgeInests、imageEdgeInsets、contentEdgeInsets 这三者的布局逻辑，sizeThatFits: 里会把 titleEdgeInests 和 imageEdgeInsets 也考虑在内（UIButton 不会），以使这三个接口的使用更符合直觉。
 */
@interface BZMButton : UIButton
...
/**
 * 设置按钮里图标和文字的相对位置，默认为BZMButtonImagePositionLeft<br/>
 * 可配合imageEdgeInsets、titleEdgeInsets、contentHorizontalAlignment、contentVerticalAlignment使用
 */
@property(nonatomic, assign) BZMButtonImagePosition imagePosition;
