...
- (void)didInitialize {
    ...
    self.imagePosition = BZMButtonImagePositionLeft;
}
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.imagePosition) {
        case BZMButtonImagePositionTop:
        case BZMButtonImagePositionBottom: {
        ...
        case BZMButtonImagePositionLeft:
        case BZMButtonImagePositionRight: {
            // 图片和文字水平排版时，高度以文字或图片的最大高度为最终高度
            // 注意这里有一个和系统不一致的行为：当 titleLabel 为多行时，系统的 sizeThatFits: 计算结果固定是单行的，所以当 BZMButtonImagePositionLeft 并且titleLabel 多行的情况下，BZMButton 计算的结果与系统不一致
...
- (void)layoutSubviews {
    ...
    if (self.imagePosition == BZMButtonImagePositionTop || self.imagePosition == BZMButtonImagePositionBottom) {
        ...
        if (self.imagePosition == BZMButtonImagePositionTop) {
    ...
    } else if (self.imagePosition == BZMButtonImagePositionLeft || self.imagePosition == BZMButtonImagePositionRight) {
        ...
        if (self.imagePosition == BZMButtonImagePositionLeft) {
            switch (self.contentHorizontalAlignment) {
                case UIControlContentHorizontalAlignmentLeft:
                    imageFrame = isImageViewShowing ? CGRectSetX(imageFrame, contentEdgeInsets.left + self.imageEdgeInsets.left) : imageFrame;
...
- (void)setImagePosition:(BZMButtonImagePosition)imagePosition {
